...
then
 ...
 qiac scan -a $URL -u $UNAME -p $PASS -d $SCANFOLDER -m json -n GitHubActionScan --policy_name "Upwork IaC Policy" --branch $GITHUB_REF --gitrepo $GITHUB_REPOSITORY --source $SOURCE_UUID > /result.json
 if [ $? -ne 0 ]; then
    exit 1
 fi
